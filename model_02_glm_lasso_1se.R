...
glm.input.1se <- as.formula(paste("LABEL", "~", paste(beta.features, collapse = "+")))

# Train GLM model
cat("Training GLM model...\n")
reviews.glm.1se <- glm(glm.input.1se, family=binomial, data = X.df)
...
predict.glm.1se <- predict(reviews.glm.1se, newdata = X.test.df, type = "response")
